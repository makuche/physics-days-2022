    ...
    for setup in FIG_LABELS:
        ...
        for measure, ax_label in zip(
            ...
            if measure == 'Totaltime':
                ...
                axs[ax_label].set_xlabel('Number of Lower-Fi Samples')
            elif (measure == 'Iterations') and (ax_label == 'a)'):
                axs[ax_label].set_ylabel('BOSS iterations')
    for setup in means:
        for measure in means[setup]:
            ...
            if measure == 'Totaltime':
                print(tmp[:, 1] / tmp[0, 1])
    fig.suptitle('Cost for global minimum inference\nup to 0.1 kcal/mol accuracy',
                 fontsize=MEDIUM_SIZE)
    plt.tight_layout()
    plt.savefig('results/tran_learn_gain.png')
